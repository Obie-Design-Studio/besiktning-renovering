...
interface PageData {
  ...
  /** Obligatoriska slugs utan egen fil men AI bedömer att kravet täcks i samma område */
  aiSatisfiedRequiredSlugs: string[]
  /** Tobias-manuellt markerade klara sektioner (nav-id) */
...
async function fetchPageData(): Promise<PageData> {
  ...
  const [uploadsResult, commentsResult, completionResult] = await Promise.all([
    supabase.from('document_uploads').select('*').order('uploaded_at', { ascending: true }),
    supabase.from('comments').select('*').order('created_at', { ascending: true }),
    supabase.from('checklist_section_completion').select('nav_id'),
  ])
  ...
  const completedNavIds = new Set<string>(
    completionResult.error
      ? []
      : (completionResult.data ?? []).map((r: { nav_id: string }) => r.nav_id),
  )
  ...
  function mandatoryStillMissing(item: (typeof CHECKLIST_ITEMS)[number]): boolean {
    ...
    if (navId && completedNavIds.has(navId)) return false
    if ((checklistUploadsBySlug[item.slug]?.length ?? 0) > 0) return false
    return !aiSatisfiedSet.has(item.slug)
  }
...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      <ChecklistSection
        uploadsBySlug={checklistUploadsBySlug}
        commentsBySlug={commentsBySlug}
        aiSatisfiedRequiredSlugs={aiSatisfiedRequiredSlugs}
        completedSectionNavIds={completedSectionNavIds}
      />
      <div id="nav-ovrig">
        <ExtraDocsSection
          uploads={extraUploads}
          comments={commentsBySlug[EXTRA_SLUG] ?? []}
          commentsBySlug={commentsBySlug}
          sectionCompleted={completedSectionNavIds.includes('nav-ovrig')}
        />
      </div>
      <UploadLog />
    </>
